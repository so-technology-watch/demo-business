 ...
 */
package org.demo.formation.librairie.bean.provider;
...
public class ImplementationServiceProvider {
	 ...
	 * 
	 * @param pattern
	 *            the implementation class name pattern (supposed to contain 2
	 *            "%s") <br>
	 *            for example "%s.impl.jdbc.%sImpl" where <br>
	 *            the first "%s" will be replaced by the interface package name
	 *            <br>
	...
	/**
	 * Builds the service implementation full class name from the given
	 * interface or concrete Class name
	 * 
	 * @param interfaceOrConcretClassName
	 * @return
	 */
	private final String buildImplementationClassName(Class<?> interfaceOrConcretClassName) {

		Package interfacePackage = interfaceOrConcretClassName.getPackage();
		String interfacePackageName = interfacePackage != null ? interfacePackage.getName() : "";
		String interfaceSimpleName = interfaceOrConcretClassName.getSimpleName();
		String result = interfaceOrConcretClassName.getName();
		if (StringUtils.isNotBlank(pattern)){
			result = String.format(pattern, interfacePackageName, interfaceSimpleName);
		}
		return result;
	}
	...
	private final <T> T createServiceInstance(Class<T> interfaceClass, Class<?> implementationClass) {
		...
		if (interfaceClass.isInstance(instance)) {
			...
			throw new RuntimeException("Class " + implementationClass.getCanonicalName() + " does not implement "
